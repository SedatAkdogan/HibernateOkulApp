...
public class MyFactoryRepository <T,ID> implements ICrud<T,ID>{
    ...
    public List<T> findByEntity(T entity) {
        List<T> result = null;
        Class cl = entity.getClass(); // entitynin class ozelliklerine geciyorum
        Field[] fl = cl.getDeclaredFields(); // class icindeki tum fieldleri bir liste icine aliyorum id,ad,soyad...
        try {
            CriteriaQuery<T> criteria = (CriteriaQuery<T>) criteriaBuilder.createQuery(t.getClass());
            Root<T> root = (Root<T>) criteria.from(t.getClass());
            criteria.select(root);
            ...
            for (int i =0; i<fl.length;i++){
                fl[i].setAccessible(true);// erismek istedigimiz alanlarin erisimini acıyoruz.Bunu unutursanız null gelecek
                /**
                 * okumakta oldugum alan null degil ise
                 * ayrica okudugum alan id degil ise
                 */
                if(fl[i].get(entity.getClass())!= null && !fl[i].get(entity).equals("id")) {
                    /**
                     * Sorgulari yazarken degiskenlerin tipi onemlidir.Mesela int bir deger icin like kullanamazsınız
                     */
            ...
            criteria.where(predicateList.toArray(new Predicate[]{})); // 34,324324,24234,2423
    ...
    public List<T> findByColumnNameAndValue(String columnName, String value) {
        ...
        Root<T> root = (Root<T>) criteria.from(t.getClass());
        criteria.select(root);
        criteria.where(criteriaBuilder.like(root.get(columnName),value)); // kullanicinin girdigi kolon adi ve degerin icerigi kontrol edilir
        return entityManager.createQuery(criteria).getResultList();
    }
